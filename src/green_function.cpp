...
double finite_difference_step(const Waveguide& wg) {
    const double characteristic_length = std::min(wg.get_params().a, wg.get_params().b);
    return std::max(1e-10, characteristic_length * 1e-3);
}
...
Complex calculate_G_NS(double x, double y, double xp, double yp, double beta, const Waveguide& wg) {
    ...
    const auto integrand = [&](double nu) {
        ...
        const double reflection = (gamma3 - gamma1) / (gamma3 + gamma1);
...
Complex calculate_dG_NS_dx_source(double x, double y, double xp, double yp, double beta, const Waveguide& wg) {
    ...
    // Nesta etapa o gradiente de G^NS é obtido por diferença central do próprio
    // kernel. A fórmula integral direta existe, mas a quadratura oscilatória para
    // suas derivadas ainda precisa de um tratamento numérico mais robusto.
